...
public class SuperRecyclerViewUtils implements OnRefreshLoadMoreListener,
    ...
    public SuperRecyclerViewUtils(Activity activity, RecyclerView mRecyclerView, SmartRefreshLayout mRefreshLayout, LoadDataLayout loadDataLayout, BaseQuickAdapter xadapter, CallBack xcallback) {
        ...
        recyclePresenter = new BaseRecyclePresenter(activity, null, EntityType.ENTITY);
        ...
        if (xadapter != null) {
            xadapter.setAnimationWithDefault(BaseQuickAdapter.AnimationType.AlphaIn);
        }
    }
    ...
    private void onFail(int errorCode, BaseResponseBean bean, Exception exception, String error) {
        if (mPage == 1) {
            hideRefresh(false);
        } else {
            hideLoadMore(false);
    ...
    public abstract static class CallBack<T extends BaseListBean> {
        ...
        /**
         * 自定义转换数据,线程内转换 就是不需要自动转化 拦截操作
         */
        public Object onConvertData(BaseResponseBean response) {
            return null;
        }
    ...
    public static class RecyclerViewHttpCallBack implements OnRequestListener<BaseResponseBean> {
        ...
        public void requestSuccess(BaseResponseBean bean) {
            try {
                ...
                if (isPaging) {
                    BaseListBean baseBean = (BaseListBean) mRecyclerViewUtils.mCallBack.onConvertData(bean);
                    if (baseBean == null) {
                ...
                mRecyclerViewUtils.onFail(0, null, e, "数据异常");
            }
        ...
        public void requestFailed(int errorCode, BaseResponseBean bean, Exception exception, String error) {
            mRecyclerViewUtils.onFail(errorCode, bean, exception, error);
        }
