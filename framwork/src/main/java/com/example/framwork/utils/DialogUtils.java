...
public class DialogUtils {
    ...
    public Dialog getBottomDialog(Context context, View view, double height) {
        Dialog dialog = new Dialog(context, R.style.Dialog_Fullscreen);
        dialog.setContentView(view);
        ...
        WindowManager.LayoutParams lp = dialogWindow.getAttributes();
        lp.width = context.getResources().getDisplayMetrics().widthPixels;
        lp.height = (int) (context.getResources().getDisplayMetrics().heightPixels * height);
        lp.alpha = 1.0f;
        dialogWindow.setAttributes(lp);
        dialogWindow.setGravity(Gravity.BOTTOM);
        dialog.setCanceledOnTouchOutside(true);
     ...
     * @return
     */
    public Dialog getBottomGridDialog(Context context, final List<? extends DialogListInfo> list, final int selectColor, final OnItemClickListener onItemClickListener) {
        final Dialog dialog = new Dialog(context, R.style.Dialog_Fullscreen);
